...
# docker buildx build --tag AlgoBlockchain --file .pi/Dockerfile --platform linux/arm64 --cache-from AlgoBlockchain:cache --cache-to AlgoBlockchain:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar AlgoBlockchain
...
sudo bash -c "echo AlgoBlockchain > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/AlgoBlockchain.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/AlgoBlockchain.img
